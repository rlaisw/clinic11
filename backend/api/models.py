...
class VisitSummary(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    patient = models.ForeignKey(
        'Patient', on_delete=models.CASCADE, related_name='visit_summaries'
    )
    doctor_name = models.CharField(max_length=255, blank=True)
    doctor_display_name = models.CharField(max_length=255, blank=True, default='')
    clinic_name = models.CharField(max_length=255, blank=True)
    ...
    visit_type = models.CharField(max_length=100, blank=True, default='')
    diagnosis = models.TextField(blank=True, default='')
    notes = models.TextField(blank=True, default='')
    status = models.CharField(max_length=10, choices=VISIT_STATUS_CHOICES, default='active')
    certificate = models.ForeignKey(
        'SickLeaveCertificate', on_delete=models.SET_NULL, null=True, blank=True, related_name='visit_summary'
    )
    receipt = models.ForeignKey(
        'Receipt', on_delete=models.SET_NULL, null=True, blank=True, related_name='visit_summary'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"VisitSummary {self.id} - {self.patient_name} ({self.visit_date})"
